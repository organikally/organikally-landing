...
export default function Product() {
  return (
    <section id="product" className="relative z-10 pt-16 pb-16 md:pt-32 md:pb-24">
        ...
        <div className="grid items-center gap-10 md:grid-cols-[0.92fr_1.08fr] md:gap-16">
          ...
          <Reveal direction="left" className="md:order-1">
            <div className="relative flex items-center justify-center">
              <span className="absolute right-0 top-2 z-10 rounded-full border border-yellow/40 bg-paper px-3 py-1 text-xs font-semibold uppercase tracking-wide text-yellow-ink">
                Organically grown
              </span>
              ...
              <img
                ...
                width={520}
                height={680}
                className="max-h-[380px] w-auto mix-blend-multiply md:max-h-[560px]"
              />
            </div>
          </Reveal>
          ...
          <Reveal className="md:order-2">
            ...
            <ul className="mt-9 max-w-md divide-y divide-line">
              {benefits.map((b) => (
                <li key={b} className="flex items-center gap-3 py-3.5 text-[1.0625rem] text-ink">
                  <Check className="h-[18px] w-[18px] shrink-0 text-yellow-deep" strokeWidth={2.4} aria-hidden="true" />
                  {b}
                </li>
              ))}
            </ul>
