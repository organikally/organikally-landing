...
CROP="crop=1600:1080:0:0"  # drop the right 1/6 (1920→1600) so the centred bottle sits right-of-centre
DESKTOP_W=1600      # post-crop native width — the video is the centrepiece, render at top quality
MOBILE_W=1280
...
ffmpeg -y -hide_banner -loglevel error -i "$SRC" \
  -vf "crop=1600:1080:0:0,scale=${DESKTOP_W}:-2:flags=lanczos" -start_number 1 "$WORK/d/%04d.png"

echo "→ extract mobile PNGs @${MOBILE_W}px (every ${MOBILE_STRIDE})"
ffmpeg -y -hide_banner -loglevel error -i "$SRC" \
  -vf "crop=1600:1080:0:0,scale=${MOBILE_W}:-2:flags=lanczos,select=not(mod(n\,${MOBILE_STRIDE}))" \
...
ffmpeg -y -hide_banner -loglevel error -i "$SRC" \
  ...
  -c:v libx264 -crf 24 -pix_fmt yuv420p -vf "crop=1600:1080:0:0,scale=1280:-2:flags=lanczos" \
  "$OUT/hero-scrub.mp4"
